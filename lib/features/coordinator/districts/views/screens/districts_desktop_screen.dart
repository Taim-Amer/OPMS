...
import 'package:opms/features/coordinator/districts/views/widgets/districts_item.dart';
...
class DistrictsDesktopScreen extends GetView<DistrictsController> {
  const DistrictsDesktopScreen({super.key, required this.fromAnother});
      ...
      desktop: TRoundedContainer(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              TextWidget(
                text: 'Districts'.s16w700,
                fontWeight: FontWeight.w700,
                fontSize: 24,
              ),
              Sizes.spaceBtwSections.verticalSpace,
              Expanded(
                child: GetBuilder<DistrictsController>(
                  builder: (controller) {
                    final districts = controller.districtsModel.data ?? [];

                    return Column(
                      children: [
                        Expanded(
                          child: Skeletonizer(
                            enabled: controller.getDistrictsState == RequestState.loading,
                            child: StaggeredGrid.count(
                              ...
                              children: List.generate(
                                districts.length,
                                    (index) {
                                  final district = districts.reversed.elementAt(index);
                                  return StaggeredGridTile.count(
                                    crossAxisCellCount: 1,
                                    mainAxisCellCount: .4,
                                    child: TSlideAnimation(
                                      beginOffset: Offset(0, index.isEven ? 1 : -1),
                                      child: DistrictItem(district: district),
                                    ),
                                  );
                                },
                              ),
                        ...
                        TPaginationControls(
                          totalItemCount: controller.districtsModel.meta?.total ?? 0,
                          ...
                          onPrevious: () {
                            if (controller.currentPage > 1) {
                              controller.getDistricts(page: controller.currentPage - 1);
                            }
                          },
                          onNext: () {
                          ...
                          onPerPageChanged: (newPerPage) {
                            ...
                            controller.getDistricts(page: 1, perPageOverride: newPerPage);
                          },
                        ),
                      ],
                    );
