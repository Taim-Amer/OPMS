...
class GovernoratesDesktopScreen extends GetView<GovernorateController> {
  const GovernoratesDesktopScreen({super.key});
                  ...
                  children: [
                    Expanded(
                      ...
                      child: StaggeredGrid.count(
                        crossAxisCount: HelperFunctions.isTabletScreen(context) ?  3 : 5,
                        mainAxisSpacing: 12,
                        crossAxisSpacing: 12,
                        // axisDirection: AxisDirection.right,
                        children: List.generate(
                          controller.governorateModel.data?.length ?? 0,
                              (index) => StaggeredGridTile.count(
                            crossAxisCellCount: 1,
                            // mainAxisCellCount: index.isEven ? .8 : 1,
                            // mainAxisCellCount: 1 ,
                            //     mainAxisCellCount: index.isEven ? 0.4 : 0.45,
                                mainAxisCellCount: .4,

                                child: TSlideAnimation(
                              beginOffset: Offset(0, index.isEven ? 1 : -1),
                              child: Skeletonizer(
                                enabled: controller.getGovernoratesState == RequestState.loading || controller.governorateModel.data == null || !Get.isRegistered<GovernorateController>(),
                                child: GovernoratesItem(
                                  governorate: controller.governorateModel.data?[index],
                                ),
                              ),
                            ),
                          ),
