// lib/features/planner/planner_home/view/widgets/planner_sidebar.dart
...
class PlannerSidebar extends StatefulWidget {
  const PlannerSidebar({super.key});
...
class _PlannerSidebarState extends State<PlannerSidebar> {
  ...
  void _onSectionTap(int idx, _SectionEntry sec, bool isMobile) {
    ...
    if (!isMobile && currentlySelected) {
      ...
      // otherwise double-tap toggles collapse/expand
      setState(() => _collapsed = !_collapsed);
    } else if (!isMobile && _collapsed) {
      // any other tap while collapsed first expands
      setState(() => _collapsed = false);
    }

    // perform navigation unless we just toggled collapse for details->active
    if (!(onDetails && currentlySelected && idx == 0)) {
  ...
  Widget build(BuildContext context) {
    ...
    // fixed full and collapsed widths
    final fullWidth =
    ...
    final sections = <_SectionEntry>[
      const _SectionEntry(
        ...
        routeName: 'plannerActive',
      ),
      const _SectionEntry(
        icon: Icons.add,
        label: 'Show available activities',
        routeName: 'plannerAvailable',
      ),
      const _SectionEntry(
        icon: Icons.account_tree_outlined,
        label: 'Tree',
        routeName: 'plannerTree',
      ),
      const _SectionEntry(
        icon: Icons.hourglass_full,
        label: 'Archive',
        routeName: 'plannerArchive',
      ),
      const _SectionEntry(
        icon: Icons.notifications_outlined,
        label: 'Notifications',
        routeName: 'plannerNotifications',
      ),
    ];
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: TextWidget(
                        text: 'Planner Name'.s14w700(context),
      ...
      return Column(
        ...
        children: [
          ...
          ...sections.asMap().entries.map((e) {
            ...
            // background circle & icon color logic
    ...
    // ─── wrap content & mobile vs desktop ─────────────────
...
class PlannerSidebarItem extends StatelessWidget {
  final IconData icon;
  final String label;
  final bool isSelected;
  final VoidCallback onTap;

  const PlannerSidebarItem({
    super.key,
    required this.icon,
    required this.label,
...
// ─── unchanged settings sheet ─────────────────────────────

void _showSettings(BuildContext context, bool isMobile) {
  final themeCtrl = Get.find<ThemeController>();
  final plannerCtrl = PlannerController.instance;
