...
class ManagerArchivedActivitiesSection extends StatelessWidget {
  const ManagerArchivedActivitiesSection({super.key});
      ...
      final table = DataTable(
        ...
        rows: List.generate(items.length, (i) {
          ...
          return DataRow(
            ...
            cells: [
              DataCell(Text(a.activity.code)),
              DataCell(Text(a.activity.name)),
              DataCell(SizedBox(
                width: 100.w,
                child:
                    Center(child: _buildStatusBadge(a.status ?? '—', context)),
              )),
              DataCell(Text(a.statusBy.toString() ?? '—')),
              DataCell(Container(
                decoration: BoxDecoration(
            ...
            child: Obx(() {
              switch (ctrl.archivedLoadState.value) {
                ...
                case RequestState.success:
                  final items = ctrl.archivedActivities.value;
                  if (items.isEmpty) {
                    return Center(
                      child: TextWidget(
